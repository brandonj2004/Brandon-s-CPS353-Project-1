...
public class Checkpoint1TestSuite {
  ...
  private Date getCommitDate(JsonElement c1) throws ParseException {
    return new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss'Z'")
            .parse(c1.getAsJsonObject().get("commit").getAsJsonObject()
            .get("committer").getAsJsonObject().get("date").getAsString());
  }

  // parse check names and results from the json
  private Map<String, String> getStatusCheckResult(String baseApiPath, JsonElement commit)
          throws Exception {
    ...
    for (JsonElement check : JsonParser.parseString(statusCheckResult).getAsJsonObject()
